  ...
  const handleSubmit = async () => {
    ...
    if (mode === 'login') {
      try {
        ...
        setErrorMessage(err.message || 'Credenciales inválidas. Por favor verifica.');
      } finally {
        setLoading(false);
      }
    } else {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 409 || data.error?.includes('already exists')) {
            ...
            throw new Error('Este correo ya está registrado en LeFriApp.');
          }
        ...
        toast({
          ...
          description: "Bienvenido a la red de justicia digital de Fundación Underlife.",
        });
        setTimeout(() => {
          setLocation('/dashboard');
        }, 800);
